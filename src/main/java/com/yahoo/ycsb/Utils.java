...
public class Utils {
  ...
  /**
   * Generate a random ASCII string of a given length seeded by key and field numbers.
   */
  public static String testableData(int key, int field, int length) {
    int salt = (102013 * saltCounter.getAndIncrement()) & (0x7FFF);
    return testableData(key, field, length, salt);
  }
  ...
  public static String testableData(int key, int field, int length, int salt) {
    ...
    return salt + "-" + new String(buf);
  }

  public static boolean checkData(int key, int field, String actual) {
    if (actual != null && actual.indexOf('-') > 0) {
      int saltSize = actual.indexOf('-');
      int salt = Integer.parseInt(actual.substring(0, saltSize));
      String replica = testableData(key, field, actual.length() - saltSize - 1, salt);
      return actual.equals(replica);
    } else {
      return false;
    }
  }
